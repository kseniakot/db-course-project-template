...
def product_reviews(product_id: str, limit: int = 20) -> list[dict[str, Any]]:
    ...
    return list(collection.find({"product_id": product_id}, {"_id": 0}).sort("helpful_votes", -1).limit(limit))


@router.get("/products/{product_id}/specs")
def product_specs(product_id: str) -> dict[str, Any]:
    """Specifications for a product."""
    doc = mongo_client.get_collection("product_specs").find_one({"product_id": product_id}, {"_id": 0})
    if doc is None:
...
def seller_profile(seller_id: str) -> dict[str, Any]:
    ...
    doc = mongo_client.get_collection("seller_profiles").find_one({"seller_id": seller_id}, {"_id": 0})
    if doc is None:
...
def user_preferences(user_id: str) -> dict[str, Any]:
    ...
    doc = mongo_client.get_collection("user_preferences").find_one({"user_id": user_id}, {"_id": 0})
    if doc is None:
